-- =====================================================
-- CORREÇÃO CRÍTICA: Políticas RLS Inseguras
...
USING (
    EXISTS (
        SELECT 1 FROM profiles p
        WHERE p.id = auth.uid()
        AND p.team_id = ml_jogador_posicoes.team_id
    )
    OR is_super_admin(auth.uid())
);
...
-- Política SELECT: Usuários veem apenas suas próprias notificações OU do seu time (se admin)
