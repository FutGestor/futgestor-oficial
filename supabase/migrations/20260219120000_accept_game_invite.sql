-- Function to accept game invite and guarantee sync
-- This function runs with SECURITY DEFINER to bypass RLS and ensure we can
-- update/insert the 'times' records for BOTH teams to ensure they are linked to 'Live' data.
...
BEGIN
  ...
  SET 
    ...
    cidade = v_team_b_data.cidade, -- Assuming these columns exist from recent migrations
    estado = v_team_b_data.estado,
    updated_at = now()
